...
//Code Here
let postTaxPrices = prices.map( prices => prices * 1.07)  // = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation =  populations.reduce( ( populations, cv, ci, arr) => {
  return populations + cv
})
...
let orderTotals = orders.map(orders => orders['price'] + orders['price'] * orders['tax'])  // Code here
...
let bobsTotal = purchases.reduce((purchases, own) => {
  if(own['owner'] === 'Bob'){
    purchases += own['price'];
  }
  return purchases;
